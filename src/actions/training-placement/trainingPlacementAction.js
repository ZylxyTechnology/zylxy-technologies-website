...
export async function submitTrainingPlacementAction(prevState, formData) {
  ...
  try {
    ...
    if (payload.selectedApps.length === 0) {
      errors.selectedApps =
        "Please select at least one training or placement service target.";
    }
    ...
    const formId = "80d4d7f8-6c0d-4db3-900f-3fd54b672174";
      ...
      `https://api-na2.hsforms.com/submissions/v3/integration/submit/${portalId}/${formId}`,
      {
        ...
        body: JSON.stringify({
          ...
          context: {
            ...
            pageUri: "https://zylxytech.com/services/training-and-placement",
            pageName: "Training & Placement Solutions Intake Portal",
          },
          legalConsentOptions: {
            consent: {
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        errors: { global: "HubSpot proxy node sync failure: " + errorText },
        payload,
      };
    }
    ...
    return {
      success: false,
      errors: { global: "Server logic mesh linkage interface timeout." },
      payload,
    };
  }
}
