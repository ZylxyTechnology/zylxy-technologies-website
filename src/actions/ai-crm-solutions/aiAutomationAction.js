...
export async function submitAiAutomationAction(prevState, formData) {
  ...
  try {
    ...
    if (payload.selectedApps.length === 0) {
      errors.selectedApps =
        "Please select at least one automation service field.";
    }
    ...
    if (!response.ok) {
      ...
      return {
        success: false,
        errors: {
          global: "HubSpot data submission error context: " + errorText,
        },
        payload,
      };
    }
    ...
    return {
      success: false,
      errors: {
        global: "Server architecture tracking connection sequence timeout.",
      },
      payload,
    };
  }
