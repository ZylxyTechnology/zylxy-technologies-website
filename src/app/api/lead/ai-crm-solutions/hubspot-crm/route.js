...
export async function POST(request) {
  try {
    ...
    if (!orgType || orgType.trim().length < 1)
      errors.orgType = "Organization type required.";
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          errors: { global: "HubSpot server rejection: " + errorText },
        },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        errors: {
          global: "Internal pipeline coupling logic integration breakdown.",
        },
      },
      { status: 500 },
    );
