...
export async function POST(request) {
  try {
    ...
    if (!selectedApps || selectedApps.length === 0)
      errors.selectedApps =
        "Select at least one placement or training field target.";
      ...
      return NextResponse.json(
        {
          success: false,
          errors: {
            global: "HubSpot route proxy node exception: " + errorText,
          },
        },
        { status: 400 },
      );
    ...
    return NextResponse.json(
      {
        success: false,
        errors: {
          global: "Internal logic mesh linkage pipeline structural failure.",
        },
      },
      { status: 500 },
    );
